...
import { generateAttestationPaiement, generateFicheInscription, generateConvocation, downloadBlob } from '@/lib/utils/pdf-generator';
...
function ResultatsSection({
  ...
  const handleUploadAttestation = useCallback(async (examenId: number, file: File) => {
    ...
    try {
      ...
      // If signed URL uses token-based upload
      if (token) {
        await fetch(signedUrl, {
          method: 'PUT',
          headers: { 'Content-Type': file.type, 'x-upsert': 'true' },
  ...
  const handleDownload = useCallback(async (examenId: number, path: string) => {
    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className="divide-y divide-slate-100">
        {examens.map((examen) => {
          ...
          const diplomeLabel = getDiplomeLabel(examOptions, examen.diplome);

          return (
            <div key={examen.id} className="flex items-center gap-4 px-5 py-3.5">
              {/* Date */}
              <div className="shrink-0 w-24">
                <p className="text-xs text-slate-500">
                  {examen.dateExamen ? formatDateFr(examen.dateExamen) : 'Date non définie'}
                </p>
              </div>

              {/* Type examen */}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-800 truncate">{diplomeLabel}</p>
              </div>

              {/* Résultat badge */}
              <span className={`shrink-0 inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${resultatConfig.bg} ${resultatConfig.text}`}>
                {resultatConfig.label}
              </span>

              {/* Attestation upload/download */}
              <div className="shrink-0 flex items-center gap-1.5">
                {examen.pdfAttestationReussite ? (
                  <button
                    onClick={() => handleDownload(examen.id, examen.pdfAttestationReussite!)}
                  ...
                  <button
                    ...
                    title="Uploader une attestation"
                  >
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (file) {
                      handleUploadAttestation(examen.id, file);
                      e.target.value = '';
                    }
                  }}
                />
                {/* Re-upload button when attestation exists */}
                {examen.pdfAttestationReussite && (
                  <button
                    onClick={() => fileInputRefs.current[examen.id]?.click()}
                    disabled={uploadingId === examen.id}
...
export default function InscriptionDetail({ id }: InscriptionDetailProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Section Résultats */}
      {examens.filter((e) => e.resultat !== 'a_venir').length > 0 && (
        <ResultatsSection
          examens={examens.filter((e) => e.resultat !== 'a_venir')}
          examOptions={examOptions}
          onRefresh={() => inscription && fetchExamens(inscription.clientId, inscription.email)}
        />
      )}
                  ...
                  >
                    ...
                    {isEditing ? (
                      ...
                      <>
                        ...
                        {/* Boutons de génération / téléchargement de documents - seulement si configuré */}
                        {configured && (
                          <div className="mt-3 pt-3 border-t border-slate-200 space-y-2">
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap items-center gap-2 mt-1">
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  <button
                                    disabled={generatingDoc === `dl_convocation_${examen.id}`}
                                    onClick={async () => {
                                      if (!inscription) return;
                                      const docKey = `dl_convocation_${examen.id}`;
                                      setGeneratingDoc(docKey);
                                      try {
                                        if (examen.pdfConvocation) {
                                    ...
                                    className="p-1.5 text-purple-500 hover:bg-purple-100 rounded-md transition-colors disabled:opacity-50"
                                    title="Télécharger le PDF"
                                  >
                                    {generatingDoc === `dl_convocation_${examen.id}` ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Download className="h-3.5 w-3.5" />}
                                  </button>
                                </div>
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  <button
                                    disabled={generatingDoc === `dl_fiche_${examen.id}`}
                                    onClick={async () => {
                                      if (!inscription) return;
                                      const docKey = `dl_fiche_${examen.id}`;
                                      setGeneratingDoc(docKey);
                                      try {
                                        if (examen.pdfFicheInscription) {
                                    ...
                                    className="p-1.5 text-blue-500 hover:bg-blue-100 rounded-md transition-colors disabled:opacity-50"
                                    title="Télécharger le PDF"
                                  >
                                    {generatingDoc === `dl_fiche_${examen.id}` ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Download className="h-3.5 w-3.5" />}
                                  </button>
                                </div>
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  <button
                                    disabled={generatingDoc === `dl_attestation_${examen.id}`}
                                    onClick={async () => {
                                      if (!inscription) return;
                                      const docKey = `dl_attestation_${examen.id}`;
                                      setGeneratingDoc(docKey);
                                      try {
                                        if (examen.pdfAttestationPaiement) {
                                    ...
                                    className="p-1.5 text-emerald-500 hover:bg-emerald-100 rounded-md transition-colors disabled:opacity-50"
                                    title="Télécharger le PDF"
                                  >
                                    {generatingDoc === `dl_attestation_${examen.id}` ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Download className="h-3.5 w-3.5" />}
                                  </button>
                                </div>
                              </div>
                            </div>
                            ...
                            {/* Anciennes versions */}
                            {examen.pdfVersions && examen.pdfVersions.length > 0 && (
                              <div className="space-y-1.5">
                                {[...examen.pdfVersions].sort((a, b) => b.version - a.version).map((pv) => (
                                  <div key={pv.version} className="opacity-50">
                                    <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">
                                      Version {pv.version} — {new Date(pv.date).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                    </span>
                                    <div className="flex flex-wrap items-center gap-2 mt-1">
                                      {pv.convocation && (
                                        <button
                                          onClick={async () => {
                                            try {
                                              const res = await fetch(`/api/admin/examens/${examen.id}/download-pdf?path=${encodeURIComponent(pv.convocation!)}`);
                                              const data = await res.json();
                                              if (data.url) window.open(data.url, '_blank');
                                            } catch (err) { console.error('Erreur téléchargement:', err); }
                                          }}
                                          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-500 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors"
                                        >
                                      ...
                                      {pv.ficheInscription && (
                                        <button
                                          onClick={async () => {
                                            try {
                                              const res = await fetch(`/api/admin/examens/${examen.id}/download-pdf?path=${encodeURIComponent(pv.ficheInscription!)}`);
                                              const data = await res.json();
                                              if (data.url) window.open(data.url, '_blank');
                                            } catch (err) { console.error('Erreur téléchargement:', err); }
                                          }}
                                      ...
                                      {pv.attestationPaiement && (
                                        <button
                                          onClick={async () => {
                                            try {
                                              const res = await fetch(`/api/admin/examens/${examen.id}/download-pdf?path=${encodeURIComponent(pv.attestationPaiement!)}`);
                                              const data = await res.json();
                                              if (data.url) window.open(data.url, '_blank');
                                            } catch (err) { console.error('Erreur téléchargement:', err); }
                                          }}
